...
class IntermediateDecoder(nn.Module):
    ...
    def forward(self, ft_, f0, f1, flow0_in, flow1_in, conv_mask=None):
        ...
        if conv_mask is not None:
            odflow0, odflow1 = torch.chunk(out[:, :4, ...].clone(), 2, 1)
            out *= conv_mask
        dflow0, dflow1 = torch.chunk(out[:, :4, ...], 2, 1)
        ft_ = out[:, 4:, ...]
        flow0 = dflow0 + 2.0 * resize(flow0_in, scale_factor=2.0)
        flow1 = dflow1 + 2.0 * resize(flow1_in, scale_factor=2.0)
        if conv_mask is not None:
            return flow0, flow1, ft_, dflow0, dflow1, odflow0, odflow1
        else:
            return flow0, flow1, ft_, dflow0, dflow1,



class MyIntermediateDecoder(nn.Module):
    def __init__(self, in_ch, out_ch, skip_ch, add_ch) -> None:
        super().__init__()
        self.convblock = nn.Sequential(
            convrelu(in_ch*2+4+add_ch, in_ch*3),
            ResBlock(in_ch*3, skip_ch),
            nn.ConvTranspose2d(in_ch*3, out_ch+4, 4, 2, 1, bias=True)
        )
