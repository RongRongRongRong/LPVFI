...
def multi_flow_combine(comb_block, img0, img1, flow0, flow1, 
        ...
        img0_warp = warp(img0, flow0)
        img1_warp = warp(img1, flow1)
        imgt_merge = mask * img0_warp + (1 - mask) * img1_warp + mean + img_res
        return imgt_merge
         '''
...
class MultiFlowDecoder(nn.Module):
    ...
    def forward(self, ft_, f0, f1, flow0, flow1, conv_mask=None):
        n = self.num_flows
        f0_warp = warp(f0, flow0)
        f1_warp = warp(f1, flow1)
        out = self.convblock(torch.cat([ft_, f0_warp, f1_warp, flow0, flow1], 1))
        if conv_mask is not None:
            odflow0, odflow1, _, _ = torch.split(out.clone(), [2*n, 2*n, n, 3*n], 1)
            out *= conv_mask
        ...
        if conv_mask is not None:
            return flow0, flow1, mask, img_res, delta_flow0, delta_flow1, odflow0, odflow1
        else:
            return flow0, flow1, mask, img_res, delta_flow0, delta_flow1