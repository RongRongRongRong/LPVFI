...
class MySmallUpdateBlock(nn.Module):
    def __init__(self, cdim, odim, hidden_dim, flow_dim, corr_dim, fc_dim,
                 corr_levels=4, radius=3, scale_factor=None):
        super(MySmallUpdateBlock, self).__init__()
        cor_planes = corr_levels * (2 * radius + 1) ** 2
        self.scale_factor = scale_factor
        ...
        self.feat_head = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Conv2d(hidden_dim, odim, 3, padding=1),
        )
...
class BidirCorrBlock:
    ...
    def corr(fmap1, fmap2):
        ...
        #print(fmap1.shape[2]*fmap1.shape[2]*fmap1.shape[1]/1e9)
