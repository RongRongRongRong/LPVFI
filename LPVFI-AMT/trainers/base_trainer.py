...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.resume_epoch, self.config['epochs']):
            ...
            for data in self.loader_train:
                ...
                if epoch<self.config['switch_epochs']:
                    results = self.model.forwardS(**data)
                else:
                    results = self.model.forwardS2(**data)
                total_loss = torch.tensor(0., device=self.config['device'])
                for name, loss in self.loss_dict.items():
                    if epoch>=self.config['switch_epochs'] and name=="l_flo":
                        continue
                    l = loss(**results, **data)
                    loss_group.update({name: l.cpu().data})
                    total_loss += l
                if epoch<self.config['switch_epochs']:
                    total_loss += results['loss_dft'] + results['loss_sflow'] + results['loss_rflow'] + results['loss_l1flow']
                else:
                    total_loss += results['lossl1_flow']
    ...
    def evaluate(self, epoch):
        ...
        for i, data in enumerate(self.loader_val):
            ...
            with torch.no_grad():
                if epoch<self.config['switch_epochs']:
                    results = self.model.forwardS(**data, eval=True)
                else:
                    results = self.model.forwardS2(**data, eval=True)
