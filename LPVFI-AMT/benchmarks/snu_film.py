...
for split in splits:
    ...
    psnr_list = []; ssim_list = []
    for name in pbar:
        img0 = img2tensor(read(osp.join(root+"/test",parse_path(name[0])))).to(device)
        imgt = img2tensor(read(osp.join(root+"/test", parse_path(name[1])))).to(device)
        img1 = img2tensor(read(osp.join(root+"/test", parse_path(name[2])))).to(device)
        padder = InputPadder(img0.shape, divisor)
        img0, img1 = padder.pad(img0, img1)

        embt = torch.tensor(1/2).float().view(1, 1, 1, 1).to(device)
        imgt_pred, FModel = model.get_dynamic_macs(img0, img1, embt, scale_factor=scale_factor, thres=args.thres)
        F_sum += FModel/len(pbar)
